...
import cdu.zch.tansci.domain.system.TaskLog;
import cdu.zch.tansci.mapper.system.TaskLogMapper;
import cdu.zch.tansci.service.system.TaskLogService;
    ...
    public IPage<TaskLog> page(Page page, TaskLog log) {
        ...
        if (Objects.nonNull(iPage.getRecords()) && iPage.getRecords().size() > 0) {
            iPage.getRecords().forEach(item -> {
                item.setStatusName(Objects.nonNull(item.getStatus()) ? Enums.getValueByGroup(item.getStatus(), "task_log_status") : null);
            });
        }
        return iPage;
    }
