...
import cdu.zch.tansci.domain.system.*;
import cdu.zch.tansci.domain.system.dto.SysRoleDto;
import cdu.zch.tansci.domain.system.vo.SysMenuRoleVo;
import cdu.zch.tansci.mapper.system.SysRoleMapper;
import cdu.zch.tansci.service.system.*;
import cdu.zch.tansci.utils.SecurityUserUtils;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements SysRoleService {
    ...
    public IPage<SysRole> page(Page page, SysRole role) {
        ...
        roleIPage.getRecords().forEach(item -> {
            Optional<SysDic> sOptional = statusList.stream().filter(s -> s.getDicValue().equals(item.getStatus())).findFirst();
            if (sOptional.isPresent()) {
                item.setStatusName(sOptional.get().getDicLabel());
            }
            item.setTypeName(Enums.getValueByGroup(item.getType(), "role_type"));
        });
        return roleIPage;
    }
